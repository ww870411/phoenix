## 2026-09-02 数字指挥大屏：供需拓扑实绩发货脉冲动态流线与双向悬停探查升级
...
- **功能特性与交互升级**：
  1. **已发货专线动态脉冲光流**：
     - 当鼠标悬停在供应商或施工标段上时，系统自动核验该连线是否产生过实际发运流水（保温管/管件发货量 > 0）；
     - 凡实际已发货的标段连线，叠加高亮流光带（`.flyline-stream`）、能量脉冲光波（`.flyline-pulse`）以及硬件加速顺向飞行的激光粒子光球（`<circle>` + `<animateMotion>`），两端端口自动进入 `port-beacon` 呼吸灯激活态；
  2. **未发货专线高对比度科技虚线**：
     - 未发生发货的规划连线升级为高辨识度的科技虚线（`stroke-width: 1.8px; stroke-dasharray: 6 5; opacity: 0.85;`），并随管材/管件系统渲染柔和青蓝与琥珀金底色；
     - 确保路径肉眼清晰可见，同时严格保持纯静态（无流光、无脉冲激光波、无飞行粒子），一眼看清规划归属；
  3. **双向悬停探查对称统一**：
     - 鼠标悬停在【管厂】或【标段】时，执行完全一致的发货实绩脉冲研判与联动高亮。
