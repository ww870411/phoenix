...
- 温度类指标引入权重排序：优先匹配 `平均气温/平均温度`，其次是包含“平均”+“气温/温度”关键字的指标，最后才退回其它温度选项。页面加载与“重置选择”都会根据该排序仅勾选一个温度指标，其余指标保持未选，确保趋势图与“头条简报”的相关性分析始终具备温度基准。
...
## 会话小结（2025-12-10 数据分析头条简报）

- “分析结果预览”卡片内新增“头条简报”子卡片，基于当前单位的 `previewRows`、`timeline` 与告警自动生成多段摘要（整体概览/趋势观测/相关性/风险提示），若存在“平均气温/温度”指标，会输出与其它已勾选指标的相关性（皮尔逊 r）。
