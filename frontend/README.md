...
- 图表组件 `TrendChart` 内置自适应 resize，配色含渐变填充（本期）+ 虚线（同期），依赖全局 `echarts` CDN 与输入的 timeline 数据，后端无需改动；ECharts Legend 支持点选隐藏曲线（可全不选），双轴模式下面板还会提示“左轴/右轴”所对应的指标。
- 2025-12-10 更新：`TrendChart` 在刷新 option 时改为 `notMerge: true`，解决切换指标后旧曲线残留的问题，图例/芯片的显示与曲线完全同步。
- 2025-12-10 更新：趋势图展示高度由 360px 提升至 420px，便于多指标并行观察。
