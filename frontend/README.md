...
- `pages/DataEntryView.vue` 的提交成功流程新增 3 秒倒计时提示，提示条展示“提交成功”文案并实时更新剩余秒数。
- 倒计时结束后调用 `window.location.reload()` 自动刷新页面，方便用户立即验证最新写库结果；若提交失败或离开页面会自动清理计时器。
- 样式层新增 `.submit-countdown`，保留原有成功/失败配色，倒计时提示与既有反馈条统一呈现。
