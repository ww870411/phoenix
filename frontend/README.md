...
- “生成分析结果”操作区新增“智能报告生成（BETA）”复选框，默认关闭；勾选时会在请求体附带 `request_ai_report: true`，由后端触发 AI 报告任务。按钮禁用状态与查询进度保持一致。
- 结果区右上角新增“下载智能分析报告”按钮，生成中会显示“智能报告生成中”并禁用；当后台状态 ready 时即可一键下载 Markdown 报告，按钮下方同步展示状态提示。
- 页面会依据后端返回的 `ai_report_job_id` 自动轮询 `/data_analysis/ai_report/{id}`，直至生成完成或失败，失败信息同样在结果卡片中展示。
