...
- 倒计时结束时不再整页刷新，而是调用 `reloadTemplate()` 重新拉取当前业务日期的数据，确保留在此前日历选择的表格并立刻看到最新写库结果。
- 列表会记录最近一次提交涉及的单元格，刷新完毕后临时以绿色闪烁方式高亮这些单元格 2 秒，方便人工核对写库结果。
- 新增 `.submit-countdown` 样式以及倒计时清理逻辑；失败或页面卸载会立即终止倒计时并保留原提示行为。
