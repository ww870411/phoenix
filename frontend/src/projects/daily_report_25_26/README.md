...
- `DashBoard.vue` 的 fallback 列表、`resolveSection` 调用以及边际利润/投诉/煤耗等模块同步替换文本，避免前端本地数据残留旧称谓；新的 `resolveSection` 仍兼容旧标题以便回滚。
- 若需恢复旧名称，只需撤销上述文件并重新部署，接口结构未发生变化。

## 仪表盘单耗水平布局尝试（2025-11-11）

- `useUnitConsumptionOption` 新增 `orientation` 参数，目前三张单耗卡片以 `horizontal` 方式渲染条形图，卡片高度设为 450px（较试验初始的 300px 提升 1.5 倍），栅格跨度为 `span 4` 以便三图并排展示；公司顺序固定为“研究院→庄河环海→金普热电→北方热电→金普热电→庄河环海→金普热电→北方热电→金州热电→主城区→集团汇总”（缺项自动跳过）。
...
- 全局样式统一在 `src/main.js` 引入 `./projects/daily_report_25_26/styles/theme.css`，避免刷新子路由（如 `/projects/daily_report_25_26/pages`）时主题缺失。
...
## 仪表盘设备运行状态视觉增强（2025-12-09）

- **功能增强**：将“各单位运行设备数量明细表”的纯数字展示升级为“彩色进度条 + 数字”组合。
- **视觉区分**：为“运行汽炉数”、“运行汽轮机数”等不同设备指标分配了专属主题色。
- **进度条可视化**：进度条长度根据每列（指标）的最大值自动计算，直观体现各单位在同类设备上的相对规模。
- **交互细节**：数值为 0 或无数据时，显示灰色横线 `—`。

## 仪表盘设备运行状态支持同期数据（2025-12-09）

- **功能扩展**：在“各单位运行设备数量明细表”中增加了对“同期”数据的支持。
- **后端适配**：更新 `_fill_device_status_section` 函数，使其能根据配置文件中的 `本期` 和 `同期` 列表，分别从 `value_biz_date` 和 `value_peer_date` 字段读取数据，并填充到 `section["本期数据"]` 和 `section["同期数据"]` 两个新的数据桶中。
- **前端适配**：
