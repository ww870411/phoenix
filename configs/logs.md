...
灵活日期及指标
  ...
  剔除庄河原/标煤量： 仅限数据看板/三张数据展示表中的前两张（涉及集团汇总口径的同期日/同期月累计/统计供暖期累计上），庄河自身以及各经济指标口径，未调整
  codex resume 019a5e94-f97b-7690-bd1a-6f62a13856e1
...
8.22  
  ...
  "校验规则": {
    ...
    特殊修改
    1.在数据看板的同期集团全口径耗标煤量中，剔除庄河改造4个锅炉房展示；在数据展示表1/2中，集团全口径同期耗原/标煤量剔除4个改造锅炉房。
    "sum_consumption_amount_raw_coal_zhangtun": "原煤耗量汇总(张屯)",
    "sum_consumption_std_coal_zhangtun": "标煤耗量汇总(张屯)"

    2.在数据看板及在数据展示表2中，集团全口径的供暖电单耗剔除研究院
