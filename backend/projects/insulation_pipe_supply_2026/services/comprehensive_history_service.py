...
def query_fitting_supplier_inventory_history(
    view_mode: str = "latest",  # "latest" | "history"
    start_date: Optional[date] = None,
    end_date: Optional[date] = None,
    supplier_ids: Optional[List[str]] = None,
    fitting_types: Optional[List[str]] = None,
    keyword: Optional[str] = None,
) -> Dict[str, Any]:
    """
    综合数据查询中心：供货商厂区成品【管件现货库存】综合查询。
    支持：
    1. latest 快照视图：展示各供给主体最新一次提交批次的数据，并关联前一次批次计算环比增减；
    2. history 流水视图：展示所有历史盘点流水台账；
    3. 支持按供给主体、管件大类、业务/盘点日期范围与全局关键字检索。
    """
    ...
    sup_filter_set = {s.strip().lower() for s in supplier_ids if s.strip()} if supplier_ids else None
    fitting_type_filter_set = {ft.strip() for ft in fitting_types if ft.strip()} if fitting_types else None
    kw = (keyword or "").strip().lower()
    ...
    try:
        if view_mode == "history":
            ...
            for r in rows:
                ...
                if sup_filter_set and sid.lower() not in sup_filter_set:
                    continue
                if fitting_type_filter_set and ft not in fitting_type_filter_set:
                    continue
                ...
                if kw:
                    text_corpus = f"{sid} {s_name} {ft} {mn} {ms} {batch_no} {rep_by} {remark}".lower()
                    if kw not in text_corpus:
                        continue
            ...
            inv_rows = session.execute(text("""
                SELECT 
                    id, batch_no, report_date, supply_entity_id, section_1_id,
                    fitting_type, material_name, model_spec, unit,
                    stock_qty, remark, reported_by, reported_at, updated_at
                FROM tube.tube_fitting_supplier_inventory
                WHERE batch_no = ANY(:batch_nos)
                ORDER BY supply_entity_id ASC;
            ...
            if prev_batch_numbers:
                prev_inv_rows = session.execute(text("""
                    SELECT supply_entity_id, fitting_type, material_name, model_spec, stock_qty
                    FROM tube.tube_fitting_supplier_inventory
                    WHERE batch_no = ANY(:batch_nos);
                """), {"batch_nos": prev_batch_numbers}).mappings().all()
                for pr in prev_inv_rows:
                    sid = str(pr["supply_entity_id"]).strip()
                    ft = str(pr["fitting_type"] or "").strip()
                    mn = str(pr["material_name"] or "").strip()
                    ms = str(pr["model_spec"] or "").strip()
                    prev_stock_map[(sid, ft, mn, ms)] = float(pr["stock_qty"] or 0)
            ...
            items.sort(key=lambda x: (x["supply_entity_name"], _fitting_inventory_sort_key(x)))
            latest_time_str = overall_latest_time.astimezone(BEIJING_TZ).strftime("%Y/%m/%d %H:%M:%S") if overall_latest_time else ""
