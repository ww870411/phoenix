...
def _build_data_analysis_schema_payload(
    ...
    for views in metric_view_mapping.values():
        for name in _normalize_view_list(views):
    ...
    def _resolve_view_alias(name: str) -> str:
        ...
        if candidate in available_views:
            return candidate
        # 尝试将旧命名（company_daily_analysis）映射到 analysis_company_daily
        if candidate.endswith("_analysis"):
            core = candidate[: -len("_analysis")]
            return f"analysis_{core}"
...
def _resolve_active_view_name(
    ...
    if isinstance(view_mapping, dict):
        candidate = view_mapping.get(mode_label) or view_mapping.get(mode_label.strip())
        if isinstance(candidate, dict):
            for view_name, units in candidate.items():
