...
- 说明：当前接口仍由 `services/dashboard_expression.evaluate_dashboard` 组装，字段名需保持 `直接收入`、`煤成本`、`外购热成本`、`水、电及辅材成本`、`边际利润`、`可比煤价边际利润`，否则前端将无法正确取值。
...
- 说明：请保持该节点结构中“本期”“同期”两个对象的键名及顺序稳定；若新增或更名收入科目，应同步告知前端调整映射及显示顺序。
