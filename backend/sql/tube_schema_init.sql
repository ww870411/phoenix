...
CREATE TABLE IF NOT EXISTS tube.tube_fitting_delivery (
    ...
    CONSTRAINT chk_tube_fitting_status CHECK (status IN ('pending_arrival', 'pending_receive', 'pending_warehouse', 'completed', 'cancelled'))
);
...
-- -----------------------------------------------------------------------------
-- 7. 保温直管基准表 (tube.tube_pipe_baseline)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 8. 管件与物料基准表 (tube.tube_fitting_baseline)
-- -----------------------------------------------------------------------------
